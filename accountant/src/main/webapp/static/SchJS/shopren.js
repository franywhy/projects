...
$(function () {
    $("#lbSubmit").click(function () {
        var addressid = "0"
        var book = $("#hidbook").val();
        var mymobile = $("#ordermobile").val();
        if (!$("#ordermobile").val().match(/^\d{11}$/)) {
            $("#mode_tips").html("<span class='gtl_ico_succ'></span>请输入正确的手机号码");
            openStaticPopup();
        ...
        if (book == 1) {
           addressid = $("input[name='rdbAddessChoice']:checked").val();
            if (typeof (addressid) == "undefined") {

                $("#mode_tips").html("<span class='gtl_ico_succ'></span>请添加收货地址");
                openStaticPopup();
                setTimeout("$.closePopupLayer('myStaticPopup')", 2000);
                return;
            }
        }
        var subType = $("#submitType").val();
        $.ajax({
            url: "/lib/product/ProductDetail.ashx",
            type: "POST",
            data: "type=6&subType=" + subType + "&addressid=" + addressid + "&check=" + check + "&mymobile=" + mymobile + "",
            beforeSend: function () {
