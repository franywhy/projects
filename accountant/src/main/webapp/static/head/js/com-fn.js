		...
		win.on("scroll",function(){
			if($(this).scrollTop()>=ttop){
				if(flag){
					...
					obj.after(cl);
					obj.css({"position":"fixed","width":"100%","top":0,"left":lleft,"zIndex":"1234569"});
				}
			}else{
				if(cl.length > 0){cl.remove();}
				obj.css({"position":"","zIndex":"1234568"});
				flag = true;
			}
		});
	}
...
function waitDataBefore(obj){
	$("<div class='loading-data' style='height:200px;background:url(images/icon/img-loading.gif) no-repeat center center;'></div>").appendTo(obj);
}
//数据加载完成 删除loading
