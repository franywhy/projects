...
$(document).ready(function() {
  ...
  $.ajax({
    type: "GET",
    dataType: "json",
    url: "/user/userInfo?token=" + _ticket,
    cache: false,
    success: function(result) {
      if (
        ...
        result != "undefined" &&
        result != "" &&
        (result.nickname != "" || result.mobileNo != "")
      ) {
        var username =
          result.nickname == "" ? result.mobileNo : result.nickname;
...
$(function() {
  ...
  /*===点击切换tab选项卡===*/
  $(".login-title-tab .login-tab-title").click(function() {
    var index = $(this).index();
    $(this)
      .addClass("act")
      .siblings()
      .removeClass("act");
    var oThis_tabMain = $(this)
      .parents(".login-title-tab")
      .siblings(".login-title-main")
      .find(".login-tab-main");
    oThis_tabMain
      .eq(index)
      .siblings()
  ...
  function openWindow(windowbox, e) {
    //打开的弹窗，打开的选项卡
    windowbox.fadeIn();
    $(".mask-login").fadeIn();
    $(".login-box.tologin .login-title-tab .login-tab-title")
      .eq(e)
      .click();
  }
  ...
  //注册里面的服务条款
  $(".login-tab-main .cast-p b").click(function() {
    openWindow($(".login-box.login-txt"));
  });

  //注册里面的同意按钮点击触发关闭
  $(".login-box.login-txt .iagree-btn").click(function() {
    $("#checkbox-cast").attr("checked", "true");
    $(".login-box.login-txt .close").click();
  });

  /*===忘记密码===*/
  $(".disrememberpw").click(function() {
  ...
  /*===表单验证提示信息===*/
  $(".login-box .validate").click(function() {
    var oThisAleat = $(this)
      .parents(".login-box")
      .find(".red-alert");
  ...
  $("#getCode").click(function() {
    ...
    var telReg = !!sMobile.match(/^(0|86|17951)?(1[3-9][0-9]{8}$/); //前端手机的验证
    if (telReg == false) {
  ...
  /*===重置密码===*/
  $(".login-box .repass").click(function() {
    var oThisAleat = $(this)
      .parents(".login-box")
      .find(".red-alert");
    var sMobile = $("#rMobile1").val();
  ...
  //跳转到学习中心
  $("#learning").click(function gotoLearning() {
    var _ticket = $.cookie("token");
    if (!_ticket) {
      $(".btn-login").trigger("click");
      return false;
    }
    var learningCenterUrl = $("#learningCenterUrl").val();
    //window.location.href=learningCenterUrl+_ticket;
    window.open(learningCenterUrl + _ticket); //新页面打开学习中心
  });
