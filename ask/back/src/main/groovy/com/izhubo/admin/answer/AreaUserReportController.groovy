 ...
 */
@RestWithSession
class AreaUserReportController extends BaseController {
	...
	def list(HttpServletRequest req){
		...
		List maxtime =areareportlogs().find().sort(new BasicDBObject("update_time" , -1)).limit(1).toArray();
		
				BasicDBObject maxobj = maxtime.get(0);
		
				String updatetime = maxobj.get("update_time");
		
				def result = areareportlogs().find(new BasicDBObject("update_time" ,updatetime)).toArray();
