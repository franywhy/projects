...
{
	private static Logger logger = LoggerFactory.getLogger(OpenApiV3.class);
    /**
     * 构造函数
     * @param appid 应用的ID
     * @param appkey 应用的密钥
...
//		printRespond(resp);
    ...
    public String api(String scriptName, HashMap<String, String> params, String protocol, 
    		String secret, boolean checkOpenId, 
    		int conTimeout, int soTimeout) throws OpensnsException
    {
        // 检查openid openkey等参数
        if (checkOpenId && params.get("openid") == null)
        {
            throw new OpensnsException(ErrorCode.PARAMETER_EMPTY, "openid is empty");
        }

        if (checkOpenId && !isOpenid(params.get("openid")))
        {
            throw new OpensnsException(ErrorCode.PARAMETER_INVALID, "openid is invalid");
        }
...
//        String secret = this.appkey + "&";
        ...
        // 发送请求
        String resp = SnsNetwork.postRequest(url, params, cookies, protocol, conTimeout, soTimeout);
