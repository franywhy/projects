package com.contract.controller.app;
...
import com.contract.utils.LocationUtil;
...
public class AppContractRecordController extends AbstractBaseController {
    ...
    public ResponseEntity<WrappedResponse> saveContract(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(recordList != null && recordList.size() > 0){
            ...
            for (ContractRecordPOJO contractRecordPOJO : recordList){
                JSONObject paramsJson = getZikaoParamsJson(contractRecordPOJO);
                ...
                if(contractId != null){
                    ...
                    contractRecordService.update(map1);
                    saveSigner(contractRecordPOJO.getCompanyId(),signerId,contractId);
     ...
     * @return
     */
    private boolean saveSigner(Long companyId,Long signerId,Long contractId){
        List<JSONObject> list = new ArrayList<JSONObject>();
        JSONObject appJson = new JSONObject();
        appJson.put("signerId",companyId);
    ...
    /**
     * 获取自考云合同JSON参数
     * @param contractRecordPOJO
     * @return
     */
	private JSONObject getZikaoParamsJson(ContractRecordPOJO contractRecordPOJO){
        JSONObject paramsJson = null;
        List<ContractDetailPOJO> detailList = contractDetailService.queryPojoList(contractRecordPOJO.getId());
        //生成收支项目table数组
