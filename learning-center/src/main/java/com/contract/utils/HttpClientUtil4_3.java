...
import com.school.utils.http.HttpEntityHandler;
...
//        System.out.println(HttpClientUtil.get("http://w.kjcity.com/ajax/TrainingCourse.ashx?openid=18620523707", map));
     ...
     * @return
     */
    public static JSONObject doPost1(String url, JSONObject json, String token){
        ...
        try {
            ...
            if(res.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
                ...
                if(StringUtils.isBlank(token)){
                    response.put("token",headers[12].getValue());
                }
            }
        } catch (Exception e) {
    ...
    public static JSONObject doPatch(String url,JSONObject jsonParam, String token){
        ...
        try {
            ...
            log.error("云合同接口返回对象：",response.toString());
        }
        return resultObj;
    }
