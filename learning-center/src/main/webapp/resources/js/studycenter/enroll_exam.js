!function(){
	function pageScript(){
		...
		this.getCourseList = function(){
			...
			$.ajax({
				url:"/learningCenter/web/courseAbnormallRegistration/courseListByOrderId",  
	            type:"GET",  
				async:true,
				data:{
		...
		this.checkForm = function(){
			...
			self.registerNum = $.trim($("#registerNum input").val());
			
			if(self.classType == ''){
				$("#promptText .text-se").text('请输入完整的报考申请内容，不能有为空项。');
				$("#promptText").css('display','inline-block');
				return false;
			}else if(self.course == ''){
				$("#promptText .text-se").text('请输入完整的报考申请内容，不能有为空项。');
				...
				$("#promptText").css('display','inline-block');
				return false;
			}else if(!reg.test(self.registerNum) && self.registerNum != ''){
				$("#promptText .text-se").text('报考登记号只能输入数字或字母');
				$("#promptText").css('display','inline-block');
				return false;
			}
			return true;
		 ...
		 * @param self.registerNum 报考登记号
		 */
		this.submitForm = function(){
			hqTool.showLoad(self.businessId);
			$.ajax({
				url:"/learningCenter/web/courseAbnormallRegistration/save",  
	            type:"GET",
				async:true,
				data:{
					"orderId" : self.userPlanId,
		...
		this.cancelRequset = function(){
			...
			hqTool.showLoad(self.businessId);
			$.ajax({
				url:"/learningCenter/web/courseAbnormallRegistration/updateCancel",
	            type:"GET",
				async:true,
				data:{
