...
import io.renren.entity.OrderMessageConsumer3Entity;
...
@Controller
public class OrderMessageConsumer3Controller implements ChannelAwareMessageListener {
	...
	public void onMessage(Message message, Channel channel) throws Exception {
		...
		// TODO
		OrderMessageConsumer3Entity orderMessageConsumer3Entity = null; //手动同步消费者实体类
		...
		try {
			...
			Gson gson = new Gson();
			orderMessageConsumer3Entity = gson.fromJson(body, OrderMessageConsumer3Entity.class);
			// 不存在相同的报名表
			if(0 == mallOrderService.queryOrderExist(null, orderMessageConsumer3Entity.getSign_code(), null)){
				Map<String, Object> areaMap = new HashMap<>();
				areaMap.put("areaName", orderMessageConsumer3Entity.getArea_name());
				//省份id
				Long areaId = this.mallAreaService.queryAreaIdByName(areaMap);
				
				Map<String, Object> commodityMap = new HashMap<>();
				commodityMap.put("id", orderMessageConsumer3Entity.getCommodity_id());
				...
				if(null != goodsInfo){//商品存在
					if(null != areaId){//省份存在
						...
						checkUserMap.put("mobile", orderMessageConsumer3Entity.getUser_mobile());
						Long user_id = null;
						...
						if (!this.usersService.checkMobile(checkUserMap)) {//用户不存在
							...
							user.setMobile(orderMessageConsumer3Entity.getUser_mobile());
							user.setNickName(orderMessageConsumer3Entity.getUser_name());
							...
							usersMap.put("mobile", orderMessageConsumer3Entity.getUser_mobile());
							// 判断用户是否存在
						...
						//班级
						mallOrderEntity.setClassId(classId);
						//支付状态
						...
						// 支付時間
						mallOrderEntity.setPayTime(DateUtils.parse(orderMessageConsumer3Entity.getSign_time()));
						// 订单有效期
						mallOrderEntity.setDateValidity(DateUtils.orderDateValiditySync(DateUtils.parse(orderMessageConsumer3Entity.getSign_time()), goodsInfo.getDayValidity()));
						// 优惠金额
						...
						// 创建时间
						mallOrderEntity.setCreationTime(DateUtils.parse(orderMessageConsumer3Entity.getSign_time()));
						// 最近修改用户
						...
						// nc同步code值
						mallOrderEntity.setNcCode(orderMessageConsumer3Entity.getSign_code());
						// nc同步时间
						mallOrderEntity.setSynTime(DateUtils.parse(orderMessageConsumer3Entity.getSign_time().toString()));
						// 层次ID
						mallOrderEntity.setLevelId(goodsInfo.getLevelId());
						//专业ID
						mallOrderEntity.setProfessionId(goodsInfo.getProfessionId());
						//产品线
						mallOrderEntity.setProductId(1L);
						//部门
						mallOrderEntity.setDeptId(736L);
						//业务线
						if(1 == orderMessageConsumer3Entity.getI()){
							mallOrderEntity.setBusinessId("zikao");
						}
						if(2 == orderMessageConsumer3Entity.getI()){
							mallOrderEntity.setBusinessId("kuaiji");
						}
						if(3 == orderMessageConsumer3Entity.getI()){
							mallOrderEntity.setBusinessId("xuelxuew");
						}
						...
						saveErr(body, orderMessageConsumer3Entity, 2);
					}
				}else{//商品不存在
					saveErr(body, orderMessageConsumer3Entity, 1);
				}
			}
			...
			e.printStackTrace();
			saveErr(body, orderMessageConsumer3Entity, 4);
		} finally {
			try {
				// 提交事务
	 ...
	 */
	private void saveErr(String json , OrderMessageConsumer3Entity ms , int type){
