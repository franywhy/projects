 ...
 * 展视互动直播观看日志同步
 * 算法:观看时长求和
 * @class io.renren.modules.job.task.SynchronizeLiveLogJob2.java
 * @Description:
 * @author shihongjie
 * @dete 2017年8月21日
 */
@Component("io.renren.modules.job.task.SynchronizeGenseeLiveLogJob")
								...
								try {
									...
									if(GENSEELIVE_CODE_SUCCESS.equals(((String)logResultMap.get(GENSEELIVE_CODE)))){
										...
										if(null != logList && logList.size() > 0){
                                            logger.error("SynchronizeGenseeLiveLogJob  execute  classplanLiveId={},genseeLogSize={}",classplanLiveId,logList.size());
											for(Map<String,Object> log : logList){
												try {
													...
													//判断明细是否重复-重复跳过
													if(liveLogDetailService.selectDetailCount(webcastId, liveNum, uid, joinTime, leaveTime,classplanLiveId))continue;
                                                    ...
                                                    liveLogDetailEntity.setClassplanLiveId(classplanLiveId);
